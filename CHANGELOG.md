# Changelog

All notable changes to this fixture documented per
[Keep a Changelog 1.1](https://keepachangelog.com/en/1.1.0/).

This fixture's purpose is to verify the website's release pipeline; the
"changes" are deliberately synthetic.

## [0.0.3] — 2026-05-13

### Added

- Second bare-numeral integration-test release. Verifies the new
  `rebuild-on-dispatch-staging.yml` workflow on
  blackbrowed-labs/blackbrowedlabs.com — a single dispatch should now
  refresh both production and staging in parallel without needing a
  manual `workflow_dispatch` on `deploy-staging.yml`.

## [0.0.2] — 2026-05-13

### Added

- Bare-numeral SemVer integration-test release. Verifies the website
  release pipeline accepts the canonical `X.Y.Z` tag form per the
  conventions update in blackbrowed-labs/blackbrowedlabs.com#21.

## [v0.0.1-pre] — 2026-04-29

### Added

- Initial fixture repo creation. Synthetic v0.0.1-pre to provide the
  loader with non-empty release data on dev.
  
